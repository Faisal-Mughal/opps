...
class TypingSpeedApp:
    ...
    def save_results_to_file(self, filename):
        with open(filename, 'w', newline='') as file:
            ...
            writer.writerow(['Username', 'WPM', 'Accuracy'])
            for username, user in self.users.items():
                for result in user.get_results():
                    writer.writerow([username, result['result']['wpm'], result['result']['accuracy']])
    ...
    def load_results_from_file(self, filename):
        try:
            with open(filename, 'r') as file:
                ...
                for row in reader:
                    username, wpm, accuracy = row
                    if username not in self.users:
                        self.users[username] = User(username)
                    self.users[username].add_result({'wpm': float(wpm), 'accuracy': float(accuracy)})
...
def main():
    ...
    while True:  # Corrected indentation here
        ...
        if action == 'a':
            ...
            filename = input("Enter filename to save results (e.g., typing_results.csv): ")
            app.save_results_to_file(filename)

        elif action == 'l':
            filename = input("Enter filename to load results (e.g., typing_results.csv): ")
